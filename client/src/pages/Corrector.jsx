...
const Corrector = () => {
  ...
  const [insights, setInsights] = useState({ tone: 'Neutral', word_count: 0, sentiment: 'Neutral', reading_time_sec: 0 });
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setInsights(data.insights || { tone: 'Neutral', word_count: 0, sentiment: 'Neutral', reading_time_sec: 0 });
      fetchHistory();
  ...
  return (
    <div className={`flex h-screen pt-20 bg-bg-base dark:bg-[#0B1120] overflow-hidden premium-gradient transition-all duration-700 ${isFocusMode ? 'px-4 md:px-20' : ''}`}>
      <ParticleBackground />
            ...
            <div className="p-6 space-y-8 flex-1 flex flex-col">
              ...
              <div className="space-y-4">
                ...
                <section>
                    ...
                    <div className="mt-2 p-4 bg-white/40 dark:bg-white/5 rounded-[2rem] border border-slate-200/50 dark:border-white/5 flex justify-between items-center px-6">
                         <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Reading Time</p>
                         <p className="text-[11px] font-black text-primary uppercase">~{insights.reading_time_sec}s</p>
                    </div>
                </section>
      ...
      <main className="flex-1 flex flex-col relative transition-all duration-700">
        <div 
          ref={chatContainerRef}
          className={`flex-1 overflow-y-auto p-6 md:p-12 space-y-12 scroll-smooth pb-64 ${isFocusMode ? 'max-w-4xl mx-auto' : ''}`}
        >
          {messages.length === 0 ? (
        ...
        <div className={`absolute bottom-0 left-0 w-full p-6 md:p-12 bg-gradient-to-t from-bg-base dark:from-[#0B1120] via-bg-base/95 dark:via-[#0B1120]/95 to-transparent z-20 transition-all duration-700 ${isFocusMode ? 'flex justify-center' : ''}`}>
          <div className={`w-full relative group ${isFocusMode ? 'max-w-4xl' : 'max-w-4xl mx-auto'}`}>
            <AnimatePresence>
              {inputText && (
                <div className="absolute -top-14 left-0 flex gap-2 w-full overflow-x-auto hide-scrollbar pb-2">
                   ...
                   <div className="ml-auto flex gap-2">
                        <motion.button 
                            ...
                            className={`px-4 py-2 border rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-2xl transition-all ${isFocusMode ? 'bg-primary text-white border-primary' : 'bg-white dark:bg-[#131C2F] text-slate-400 border-slate-200 dark:border-white/10 hover:text-primary'}`}
                        >
      ...
      <div className="max-w-[95%] space-y-6 w-full">
        <div className="flex items-center gap-4 ml-4">
          ...
          <div className="flex flex-col">
            <span className="text-xs font-black text-slate-900 dark:text-white uppercase tracking-[0.3em]">WordWise Correctify Intelligence</span>
        ...
        <div className="bg-white dark:bg-[#131C2F] border-2 border-slate-200/40 dark:border-white/5 p-8 md:p-12 rounded-[3.5rem] rounded-tl-none shadow-[0_40px_100px_rgba(0,0,0,0.08)] dark:shadow-[0_40px_100px_rgba(0,0,0,0.5)] relative overflow-hidden transition-all duration-500">
          ...
          <AnimatePresence mode="wait">
            {isComparing ? (
                ...
                >
                    <WordReveal text={message.corrected} changes={message.changes} />
                </motion.div>
            )}
          </AnimatePresence>
...
const WordReveal = ({ text, changes }) => {
    const words = text.split(' ');
    
    return (
        <div className="flex flex-wrap gap-x-2">
            {words.map((word, i) => {
                const change = changes?.find(c => c.corrected.toLowerCase() === word.toLowerCase() || word.includes(c.corrected));
                return (
                    <motion.span
                        key={i}
                    ...
                    >
                        {change ? (
                            <span className="relative group inline-block">
                                <span className="bg-correct/15 text-correct-dark dark:text-correct px-2 py-0.5 rounded-xl border-b-4 border-correct/30 cursor-help transition-all group-hover:bg-correct/30 group-hover:scale-105">
                                    {word}
                                </span>
